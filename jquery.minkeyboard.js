(function( factory ) {
	...
	var minkeyboardOverrides = {
		...
		_minkeyPress: function (event, keyChar) {
        ...
        _createKey: function (keyChar) {
			...
			switch (keyChar) {
				case "\x0A": keyName = "Enter";
					break;
				case "\x08": keyName = "Backspace";
					break;
				case " ": keyName = "Space";
					break;
				case "-": keyName = "Dash";
            ...
            this._on(key, {
                click: function (event) {
					this._minkeyPress(event, keyChar);
				}
            });
			return key;
        },
		...
		_create: function () {
			...
			if (this.options.keys) { // options.keys doit avoir la priorite sur options.pattern
				this._buildKeyboardFromKeyChars(this.options.keys);
			}
            // si l'element n'as pas de pattern, on lui autorise tout le keypad
			else if (!this.options.keys && !this.options.pattern) {
				this.options.pattern = this.element.attr("pattern") || '[' + _keypad + ']';
			} else {
